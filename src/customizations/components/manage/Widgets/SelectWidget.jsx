  ...
  if (isObject(value)) {
    if (Array.isArray(value)) {
      return choices.reduce((acc, choice) => {
        value.map((v) => v.value).includes(choice[0]) &&
...
class SelectWidget extends Component {
  ...
  initSelectedOptions(value) {
    if (Array.isArray(value)) {
      return value.map((v) => {
        return { label: v.title, value: v.value };
      });
    } else {
      return value ? { label: value.title, value: value.value } : {};
    }
  }

  state = {
    selectedOption: this.initSelectedOptions(
      this.props.values || this.props.value,
    ),
  };

  initOptions(choices, noValuePresent) {
    const options = [
      ...map(choices, (option) => ({
        value: option[0],
        label:
      ...
      <FormFieldWrapper {...this.props} draggable={true}>
        ...
        {this.props.vocabBaseUrl ? (
          ...
          <Select
            ...
            options={this.initOptions(choices, noValuePresent)}
            styles={customSelectStyles}
            theme={selectTheme}
            components={{ DropdownIndicator, Option }}
            defaultValue={
              id === 'roles' || id === 'groups'
                ? getDefaultValues(choices, values)
                : getDefaultValues(choices, value)
            }
            onChange={(data) => {
              let dataValue = [];
              // if "id" is roles || groups means the select is multi
              if (Array.isArray(data) || id === 'roles' || id === 'groups') {
                // check if data is iterable https://stackoverflow.com/questions/18884249/checking-whether-something-is-iterable
                if (
                  data != null &&
