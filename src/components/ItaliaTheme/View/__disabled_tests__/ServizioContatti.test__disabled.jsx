...
import ServizioContatti from '../ServizioView/ServizioContatti';
...
test('expect to render fields in page', async () => {
  render(
    <Provider store={store}>
      <MemoryRouter>
        <ServizioContatti content={mock_all_fields} />
      </MemoryRouter>
    </Provider>,
  );

  // contatti
  const contactCard = document.querySelector('#contatti .card');
  ...
  // area
  expect(
    screen.getByText(/Area impiantistica sportiva e manifestazioni sportive/i),
  ).toBeInTheDocument();
});
