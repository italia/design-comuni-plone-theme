...
import ServizioAccedi from '../ServizioView/ServizioAccedi';
...
const mock_all_fields = {
  ...
  modified: '2023-03-15T13:59:29+00:00',
...
test('expect to have all mandatory fields in page', async () => {
  render(
    <Provider store={store}>
      <MemoryRouter>
        <ServizioAccedi content={mock_mandatory} />
      </MemoryRouter>
    </Provider>,
  );
...
test('expect to render other fields in page', async () => {
  render(
    <Provider store={store}>
      <MemoryRouter>
        <ServizioAccedi content={mock_all_fields} />
      </MemoryRouter>
    </Provider>,
  );
  ...
  expect(
    screen.getByRole('link', { name: /Accedi al servizio/i }),
  ).toBeInTheDocument();

  //dove rivolgersi
  expect(
    screen.getByRole('heading', { name: /Dove rivolgersi/i }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole('link', { name: /Ufficio delle Attività Produttive/i }),
  ).toBeInTheDocument();

  // dove rivolgersi - info aggiuntive
