...
import ServizioModulistica from '../ServizioView/ServizioModulistica';
...
test('expect to render fields in page', async () => {
  render(
    <Provider store={store}>
      <MemoryRouter>
        <ServizioModulistica content={mock_all_fields} />
      </MemoryRouter>
    </Provider>,
  );

  expect(screen.getAllByRole('heading', { name: /Modulistica/i })).toBeTruthy();
  expect(
    screen.getByRole('link', { name: /I have this thing/i }),
  ).toBeInTheDocument();
  expect(
    screen.getByText(/Where I get older but just never wiser/i),
  ).toBeInTheDocument();
});
