...
import ServizioTempiScadenze from '../ServizioView/ServizioTempiScadenze';
...
test('expect to render fields in page', async () => {
  render(
    <Provider store={store}>
      <MemoryRouter>
        <ServizioTempiScadenze moment={moment} content={mock_all_fields} />
      </MemoryRouter>
    </Provider>,
  );

  expect(
    screen.getAllByRole('heading', { name: /Tempi e scadenze/i }),
  ).toBeTruthy();

  //calendar card
  const calendarDateCard = document.querySelector('.calendar-date');
  expect(calendarDateCard).toBeInTheDocument();
