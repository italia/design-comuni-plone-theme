...
const Item = ({ day, path, data, inEdit }) => {
  ...
  return (
    <div>
			...
			<div>
			<hr />
			{querystringResults.subrequests && querystringResults.subrequests[day] ? 
				(
					querystringResults.subrequests && querystringResults.subrequests[day]?.items[day]?.map((item, index) => (
						<div key={index}>
							<div className="pl-3">
								<div>
									{item?.type}
