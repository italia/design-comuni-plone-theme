...
test('View renders all fields', async () => {
  ...
  //titolo
  expect(
    screen.getByText(/Steve walks warily down the street/i),
  ).toBeInTheDocument();
  //descrizione
  expect(
  ...
  expect(
    screen.getByRole('link', {
      name: /Ain't no sound but the sound of his feet/i,
    }),
  ).toBeInTheDocument();
  //icona
  ...
  //titolo card
  expect(screen.getByText(/Machine guns ready to go/i)).toBeInTheDocument();
  //testo card
  expect(screen.getByText(/Are you ready for this?/i)).toBeInTheDocument();
  //link card
  expect(
    screen.getByRole('link', {
      name: /Are you hanging on the edge of your seat?/i,
    }),
  ).toBeInTheDocument();
});
